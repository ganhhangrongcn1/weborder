const STORAGE_KEY = "ghr_admin_request_audit_v1";
...
function writeEvents(events = []) {
  ...
  try {
    ...
    // Audit must never block admin operation.
  }
}

function notifyAuditChanged() {
  if (typeof window === "undefined") return;
  window.dispatchEvent(new CustomEvent("ghr:admin-request-audit-changed"));
}

function compactEvents(events = [], now = getNow()) {
  const from = now - HOUR_MS;
  return events.filter((event) => Number(event?.at || 0) >= from).slice(-800);
}
...
export function recordAdminRequest(scope = "unknown", table = "unknown", type = "read") {
  if (!canUseLocalStorage()) return;
...
export function getAdminRequestAuditSnapshot() {
  const now = getNow();
  const events = compactEvents(readEvents(), now);
  writeEvents(events);
...
export function resetAdminRequestAudit() {
  if (!canUseLocalStorage()) return;
