...
export default function CartItem({ item, onInc, onDec, onRemove }) {
  ...
  return (
    <CustomerCard padding="sm" className="flex gap-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-2">
          ...
          <button type="button" onClick={() => onRemove(item.cartId)} className="grid h-8 w-8 shrink-0 place-items-center rounded-full text-brown/50">X</button>
        </div>
        <div className="mt-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <CustomerButton variant="secondary" size="sm" onClick={() => onDec(item.cartId)}>-</CustomerButton>
            <span className="w-5 text-center text-sm font-bold">{item.quantity}</span>
            <CustomerButton variant="soft" size="sm" onClick={() => onInc(item.cartId)}>+</CustomerButton>
          </div>
