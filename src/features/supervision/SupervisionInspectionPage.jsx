...
function InspectionSummary({ inspection, items }) {
...
export default function SupervisionInspectionPage({ adminAuth }) {
  ...
  return <main className="supervision-check">
    ...
    {itemIndex === items.length - 1 ? <section className="inspection-closing"><div><h2>Xác nhận cuối biên bản</h2><p>Nhân viên đọc lại nội dung, có thể ghi ý kiến và xác nhận trực tiếp trên điện thoại.</p></div>{flow.inspection.participants.length ? <div className="participant-confirmations">{flow.inspection.participants.map((participant) => { const confirmation = confirmations.find((item) => item.participant_id === participant.id); const comment = confirmationComments[participant.id] ?? confirmation?.employee_comment ?? ""; return <article key={participant.id} className={confirmation ? "confirmed" : ""}><label><input type="checkbox" checked={Boolean(confirmation)} disabled={flow.working} onChange={() => toggleConfirmation(participant)} /><span><strong>{participant.employee_name_snapshot}</strong><small>{confirmation?.confirmation_method === "signature" ? "Đã ký tên" : confirmation ? "Đã xác nhận" : "Chưa xác nhận"}</small></span></label><textarea rows="2" value={comment} placeholder="Ý kiến của nhân viên nếu có…" onChange={(event) => setConfirmationComments((current) => ({ ...current, [participant.id]: event.target.value }))} onBlur={() => saveConfirmationComment(participant)} /></article>; })}</div> : <p className="no-participants">Biên bản không có nhân viên trong ca để xác nhận.</p>}{flow.inspection.participants.length ? <div className="signature-choice"><label><span>Người ký tên</span><select value={selectedSignatureParticipant?.id || ""} onChange={(event) => setSignatureParticipantId(event.target.value)}>{flow.inspection.participants.map((participant) => <option key={participant.id} value={participant.id}>{participant.employee_name_snapshot}</option>)}</select></label>{selectedSignatureParticipant ? <SignaturePad signerName={selectedSignatureParticipant.employee_name_snapshot} disabled={flow.working} onSave={saveSignature} /> : null}</div> : null}<label className="final-notes"><span>Ghi chú của người giám sát</span><textarea rows="4" value={inspectionNotes} placeholder="Ghi nhận tình hình chung, nội dung đã trao đổi hoặc hướng xử lý…" onChange={(event) => setInspectionNotes(event.target.value)} /></label>{!allParticipantsConfirmed ? <p className="supervision-error">Cần xác nhận đầy đủ nhân viên trong ca trước khi hoàn tất.</p> : null}</section> : null}
