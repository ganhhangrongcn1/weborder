    ...
    <section className="grid min-h-[calc(100vh-96px)] place-items-center px-4">
      <div className="w-full rounded-[30px] bg-success p-7 text-center shadow-soft">
        ...
        <h1 className={`mt-6 text-2xl font-black ${isConfirmed ? "text-green-700" : "text-orange-700"}`}>
          {isConfirmed ? "Đơn hàng đã được gửi xác nhận" : "Còn 1 bước để quán nhận đơn"}
        </h1>
        <p className="mt-2 text-sm font-bold text-brown/70">
          {isConfirmed ? "Bạn đã mở Zalo để gửi thông tin đơn cho quán." : "Bạn cần gửi tin nhắn Zalo để quán xác nhận và bắt đầu chuẩn bị món."}
        </p>
        ...
        <div className={`mt-4 rounded-[24px] border bg-white/85 p-4 text-left shadow-soft ${isConfirmed ? "border-green-200" : "border-orange-200"}`}>
          {!isConfirmed && (
            <p className="mb-3 rounded-2xl bg-orange-50 px-3 py-2 text-xs text-orange-700">
              Lưu ý: quán chỉ bắt đầu chuẩn bị món sau khi nhận được tin nhắn xác nhận trên Zalo.
            </p>
          )}
            ...
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-brown/60">
                {isConfirmed
                  ...
                  : hasOpenedZalo
                    ? "Zalo đã được mở. Sau khi gửi tin nhắn cho quán, quay lại đây và bấm nút xác nhận bên dưới."
                    : "Bấm nút cam phía trên để mở Zalo quán. Nội dung đơn sẽ được copy sẵn, bạn chỉ cần dán và gửi cho quán."}
              </p>
            </div>
          </div>
          ...
          {isConfirmed ? (
            ...
            <div className="mt-3 grid gap-2">
              {hasOpenedZalo ? (
                ...
                <p className="rounded-2xl bg-cream px-3 py-3 text-xs font-semibold leading-5 text-brown/60">
                  Nút xác nhận sẽ hiện sau khi bạn bấm mở Zalo, để tránh quên gửi đơn cho quán.
                </p>
              )}
            </div>
          )}
