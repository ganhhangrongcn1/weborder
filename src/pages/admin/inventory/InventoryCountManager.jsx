...
export default function InventoryCountManager({ rows = [], warehouses = [], items = [], units = [], canWrite = false, canManage = false, canCount = false, mutationStatus = "idle", mutationMessage = "", onCreateAndStart, onRecordAndSubmit, onApproveAndComplete, onCompleteApproved }) {
  ...
  return (
    <section className="inventory-list-card inventory-count-manager">
      ...
      <div className="inventory-list-toolbar"><label className="inventory-search-field"><Icon name="search" size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Tìm mã phiếu hoặc kho..." /></label><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="active">Đang cần xử lý</option><option value="all">Tất cả trạng thái</option><option value="counting">Đang kiểm</option><option value="submitted">Chờ duyệt</option><option value="approved">Đã duyệt</option><option value="completed">Hoàn tất</option></select></div>
      ...
      {modal ? <InventoryCountModal mode={modal.mode} count={modal.count} warehouses={warehouses} items={items} units={units} saving={mutationStatus === "saving"} onClose={() => setModal(null)} onCreate={onCreateAndStart} onSubmitCount={onRecordAndSubmit} onApprove={onApproveAndComplete} /> : null}
    </section>
  );
}
