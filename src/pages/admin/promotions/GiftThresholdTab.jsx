    ...
    <section className="admin-promo-split">
      <aside className="admin-promo-side rounded-[14px] border border-slate-200 bg-white p-3 shadow-sm">
        <div>
          ...
          <button type="button" className="admin-cta" onClick={() => createPromotion("gift_threshold")}>+ Tạo mới</button>
        </div>
        <div className="admin-promo-code-list">
          {giftPromos.map((promotion) => {
            const promotionProduct = activeProducts.find((product) => product.id === promotion.reward?.productId);
            ...
            return (
              <button
                key={promotion.id}
                type="button"
                className={`admin-promo-list-card ${selectedGiftPromoId === promotion.id ? "is-active" : ""}`}
                onClick={() => setSelectedGiftPromoId(promotion.id)}
              >
                <div className="mb-2 flex items-start justify-between gap-2">
                  <strong className="text-sm font-black text-slate-900">{promotion.title || promotion.name || "Tặng món theo mốc"}</strong>
                ...
                <p className="mt-1 text-xs text-slate-700">{promotionProduct?.name || "Chưa chọn món tặng"}</p>
        ...
        <div className="admin-promo-form-flow">
          ...
          <div className="flex items-center justify-end">
            <button
              ...
              onClick={() => onDeletePromotion(giftPromo.id)}
            >
              Xóa chương trình
            </button>
          </div>
