...
export default function FreeshipManager({
  ...
  if (!freeShippingPromo) {
    return (
      <section className="admin-promo-split">
        <aside className="admin-promo-side rounded-[14px] border border-slate-200 bg-white p-3 shadow-sm">
          <div>
            ...
            <button type="button" className="admin-cta" onClick={() => createPromotion("free_shipping")}>+ Tạo mới</button>
          </div>
    ...
    <section className="admin-promo-split">
      <aside className="admin-promo-side rounded-[14px] border border-slate-200 bg-white p-3 shadow-sm">
        ...
        <div className="admin-promo-code-list">
          {freeShippingPromos.map((promotion) => {
            const promotionSubtotal = Number(promotion.condition?.minSubtotal || 0);
            const promotionSupportLabel = getSupportLabel(promotion.condition?.maxSupportShipFee || 0);
            const promotionActive = promotion.active !== false;
            return (
              <button
                key={promotion.id}
                type="button"
                className={`admin-promo-list-card ${selectedFreeShippingPromoId === promotion.id ? "is-active" : ""}`}
                onClick={() => setSelectedFreeShippingPromoId(promotion.id)}
              >
                <div className="mb-2 flex items-start justify-between gap-2">
                  <strong className="text-sm font-black text-slate-900">{promotion.title || promotion.name || "Hỗ trợ ship"}</strong>
                  <span className={`rounded-full px-2 py-1 text-[10px] font-bold ${promotionActive ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>
                    {promotionActive ? "Đang bật" : "Đang tắt"}
                  </span>
                </div>
                <p className="text-xl font-black text-orange-600">Từ {promotionSubtotal.toLocaleString("vi-VN")}đ</p>
                <p className="mt-1 text-xs text-slate-700">{promotionSupportLabel}</p>
                <p className="mt-1 text-[11px] text-slate-500">{promotion.startAt || "Chưa đặt"} → {promotion.endAt || "Chưa đặt"}</p>
              </button>
            );
          })}
        </div>
        ...
        <div className="admin-promo-form-flow">
                ...
                <div className="admin-promo-active-row mt-1">
                  ...
                  <label className="admin-switch">
                    <input
                      type="checkbox"
                      checked={isActive}
                      onChange={(event) => setExclusiveFreeShippingActive(freeShippingPromo.id, event.target.checked)}
                    />
                    <span />
                  </label>
                </div>
          ...
          <div className="flex items-center justify-end">
            <button
              ...
              onClick={() => onDeletePromotion(freeShippingPromo.id)}
            >
              Xóa chương trình
            </button>
          </div>
