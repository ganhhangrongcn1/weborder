-- Existing item thresholds remain the central/default values.
-- Branch thresholds in metadata.stock_thresholds use base-unit quantities.
create or replace function private.inventory_stock_thresholds(
  p_metadata jsonb, p_minimum numeric, p_reorder numeric,
  p_warehouse_type text, p_warehouse_id uuid
) returns table(minimum_stock numeric, reorder_point numeric)
language plpgsql immutable security invoker set search_path = ''
as $$
declare
  candidate jsonb;
begin
  minimum_stock := coalesce(p_minimum, 0);
  reorder_point := coalesce(p_reorder, 0);
  if p_warehouse_type = 'branch' then
    foreach candidate in array array[
      p_metadata #> array['stock_thresholds', 'warehouses', p_warehouse_id::text],
      p_metadata #> array['stock_thresholds', 'branch']
    ] loop
      if jsonb_typeof(candidate -> 'minimumStock') = 'number'
        and jsonb_typeof(candidate -> 'reorderPoint') = 'number' then
        if (candidate ->> 'minimumStock')::numeric >= 0
          and (candidate ->> 'reorderPoint')::numeric >= 0 then
          minimum_stock := (candidate ->> 'minimumStock')::numeric;
          reorder_point := (candidate ->> 'reorderPoint')::numeric;
          exit;
        end if;
      end if;
    end loop;
...
$$;
revoke all on function private.inventory_stock_thresholds(jsonb,numeric,numeric,text,uuid) from public, anon;
grant execute on function private.inventory_stock_thresholds(jsonb,numeric,numeric,text,uuid) to authenticated, service_role;
...
visible_balances as (
  select
    ...
    item.id as item_id,
    coalesce(balance.quantity, 0) as quantity,
    coalesce(balance.average_cost, 0) as average_cost,
    balance.updated_at,
  ...
  cross join visible_items item
  cross join lateral private.inventory_stock_thresholds(item.metadata, item.minimum_stock, item.reorder_point, warehouse.warehouse_type, warehouse.id) threshold
  left join public.inventory_stock_balances balance
    on balance.warehouse_id = warehouse.id
   and balance.item_id = item.id
  where case
    when jsonb_typeof(item.metadata -> 'warehouse_ids') = 'array'
      and jsonb_array_length(item.metadata -> 'warehouse_ids') > 0
      then (item.metadata -> 'warehouse_ids') ? warehouse.id::text
    else true
  end
),
visible_lots as (
...
stock_actions as (
  ...
  where balance.quantity <= 0
    or ((balance.reorder_point > 0 and balance.quantity <= balance.reorder_point) or (balance.minimum_stock > 0 and balance.quantity < balance.minimum_stock))
),
expiry_actions as (
  select
...
warehouse_stock as (
  select
    ...
    count(balance.item_id) filter (
      where (balance.reorder_point > 0 and balance.quantity <= balance.reorder_point) or (balance.minimum_stock > 0 and balance.quantity < balance.minimum_stock)
    ) as reorder_count
  from visible_warehouses warehouse
...
select jsonb_build_object(
  ...
  'kpis', jsonb_build_object(
    ...
    'reorder_count', (select count(*) from visible_balances where (reorder_point > 0 and quantity <= reorder_point) or (minimum_stock > 0 and quantity < minimum_stock)),
