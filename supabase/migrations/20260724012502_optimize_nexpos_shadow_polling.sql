alter table public.nexpos_shadow_sync_control
  add column if not exists last_rare_sweep_at timestamptz;

update public.nexpos_shadow_hubs
set enabled = false,
    updated_at = now()
where nexpos_hub_id in ('TEST-HUB', 'test-hub-lhp', 'test-hub-tqd');
...
select cron.schedule(
  ...
  $cron$
    ...
    where timezone('Asia/Bangkok', now())::time >= time '10:00'
      and timezone('Asia/Bangkok', now())::time < time '22:30';
  $cron$
);
