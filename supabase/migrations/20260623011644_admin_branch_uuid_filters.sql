-- Admin branch UUID filters
-- Safe to rerun. Keeps branch name fallback for legacy orders without UUID.
...
as $$
  select case
    ...
    when coalesce(trim(p_branch_name), '') <> ''
      and public.normalize_order_counting_status(p_order_branch_name) = public.normalize_order_counting_status(p_branch_name)
      then true
    else false
  end;
$$;
